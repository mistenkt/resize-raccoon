...
export const removeToast = (id: number) => {
    toasts.value = produce(toasts.value, draft => draft.filter(t => t.id !== id));
}
...
const ToastSystem = () => {
    ...
    useEffect(() => {
        ...
        if (toast && toast.type !== ToastType.ERROR && toast.hideAfterMs !== 0) {
            timer = setTimeout(() => {
    ...
    return (
        <div className={`toast-system fixed z-[10] stack bottom-0 p-2 w-full ${slideOutToast !== null ? 'removing' : ''}`}>
            {toasts.value.map(toast => (
                <div key={toast.id} className={`alert shadow-md alert-${toast.type} ${slideInToast === toast.id ? 'slide-in' : ''} ${slideOutToast === toast.id ? 'slide-out' : ''}`}>
                    {toastIcons[toast.type]}
                    <span>{toast.message}</span>
                    <button onClick={dismissToast} className="btn btn-ghost text-base-100 btn-sm self-end">{t('toast.buttons.dismiss')}</button>
                </div>
            ))}
        </div>
    )
